  ...
  try {
    ...
    const { data: savedPlanData, error: planSaveError } = await supabase
      ...
      .insert({
        ...
        custom_race_distance_value: trainingPlan.planConfiguration.customRaceDistanceValue || null,
        custom_race_distance_units: trainingPlan.planConfiguration.customRaceDistanceUnits || null,
    ...
    for (const week of trainingPlan.weeklySchedules) {
      ...
      const runsToInsert = week.runs
        ...
        .map(run => ({
          ...
          total_duration_seconds: Math.round(run.totalDuration || 0),
          total_distance_meters: Math.round(run.totalDistance || 0),
